...
def num_check(question, low, high):  # checks the number (for levels)
    ...
    while not valid:
        ...
        # ask for a number
        response_num_check = input(question)

        # check number is more than 0
        if response_num_check == "infinite":
            return "infinite"
        try:
            response_num_check = int(response_num_check)
            if high == "infinite":
                if low < response_num_check:
                    ...
                    print(error)
                    print()
            elif low < response_num_check < high:
                return response_num_check
            else: # outputs error if input is invalid
                print(error)
                print()
        except ValueError:
            print(error)
...
def start_l_1():  # def to start level 1
    ...
    sneakey = num_check("how many rounds", 0, "infinite")
    total_rounds = sneakey
    if total_rounds == "infinite":
        total_rounds = 9999999999999999999999999999
    else:
        pass
...
def start_l_2():  # def to start level 2
    ...
    total_rounds = num_check("how many rounds", 0, "infinite")
